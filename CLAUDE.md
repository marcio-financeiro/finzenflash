...
- Card/widget "Pendências" da Home (mobile + desktop): a lista de despesas pendentes SEMPRE tem que incluir as faturas de cartão em aberto do mês (`carregarFaturasPendentesLista`/`carregarFaturasMesCorrente`), não só lançamentos de `transactions` — senão cartão com fatura aberta (ex: Nubank, Nomad) some da lista mesmo contando no total do card. No desktop isso tem DOIS pontos de carga (`recarregarPendentes()` no toggle Despesas/Receitas, e `carregarDadosDoMes()` na carga inicial/troca de mês) — os dois passam por `pendentesComFaturas()`, não duplicar a lógica de novo num dos dois só.
