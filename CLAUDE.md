...
- `js/parcelamentos.js`/`desktop/parcelamentos.js`: gráfico "Variação mês a mês" (`loadChart.js`, igual Investimentos) soma `valor_parcela` de `card_transactions` por `fatura_referencia`, separado em paga/aberta (barra empilhada). Usa `gruposParaGrafico()` — só respeita o filtro de cartão, ignora "mostrar quitadas" de propósito (o gráfico precisa da curva completa passado+futuro, não só o que ainda está em andamento).
