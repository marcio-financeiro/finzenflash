...
function renderResumo(lista) {
  ...
  document.getElementById('resumo-mes-atual-rotulo').textContent = `Comprometido ${rotuloMes(mesAtual)}`;
  document.getElementById('resumo-mes-atual').textContent = fmt.format(comprometidoMes(mesAtual));
  document.getElementById('resumo-mes-proximo-rotulo').textContent = `Comprometido ${rotuloMes(mesProximo)}`;
...
function renderLista() {
  ...
  if (lista.length === 0) {
    container.innerHTML = '<div class="lista-vazia">Nenhuma compra parcelada encontrada.</div>';
    return;
  }
      ...
      <div class="parcela-card" data-chave="${escapeHtml(g.chave)}">
        <div class="parcela-header">
          <div class="parcela-topo">
            <div>
              <div style="font-weight:700">${escapeHtml(g.descricao)}</div>
...
async function recarregar() {
  try {
    ...
    console.error(err);
    document.getElementById('lista-parcelamentos').innerHTML = '<div class="lista-vazia">Não foi possível carregar os parcelamentos.</div>';
  }
}
