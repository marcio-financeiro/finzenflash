  ...
  document.getElementById('lista-metricas').innerHTML = metricas.map((m) => `
    <div class="metrica">
      <div class="metrica-linha">
        <div>${m.nome}</div>
        <div style="color:${corNota(m.nota)}">${m.nota}</div>
      </div>
...
async function iniciar() {
  ...
  try {
    ...
    console.error(err);
    document.getElementById('lista-metricas').innerHTML = '<div class="lista-vazia">Não foi possível calcular seu score agora.</div>';
  }
}
