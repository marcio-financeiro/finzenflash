...
function renderProjecao({ saldoInicial, eventos, hoje, fim }) {
  ...
  if (eventos.length === 0) {
    listaEl.innerHTML = '<div class="lista-vazia">Nada agendado nesse período — o saldo permanece o mesmo.</div>';
  } else {
    listaEl.innerHTML = eventos.map((ev) => `
      <div class="evento-item">
        <div class="evento-ponto" style="background:${ev.valor >= 0 ? 'var(--success)' : 'var(--danger)'}"></div>
        <div class="evento-info">
          <div>${escapeHtml(ev.nome)}</div>
          <div class="evento-data">${fmtData(ev.data)}${ev.recorrente ? ' · recorrente' : ev.tipo === 'fatura' ? ' · fatura' : ''}</div>
        </div>
        <div class="evento-direita">
          <div class="num ${ev.valor >= 0 ? 'positivo' : 'negativo'} valor-sensivel">${ev.valor >= 0 ? '+' : ''}${fmt.format(ev.valor)}</div>
          <div class="evento-saldo valor-sensivel">${fmt.format(ev.saldoApos)}</div>
        </div>
      </div>
    `).join('');
  }
...
async function carregarTudo() {
  ...
  document.getElementById('lista-eventos').innerHTML = '<div class="lista-vazia">Carregando...</div>';
  ...
  try {
    ...
    console.error(err);
    document.getElementById('lista-eventos').innerHTML = '<div class="lista-vazia">Não foi possível calcular a projeção.</div>';
  }
}
