...
async function mudarMes(delta) {
  ...
  renderMes();
  await Promise.all([recarregarTimeline(usuarioAtual), recarregarResumoMensal(), recarregarCardPendentes(), recarregarCardCartoes()]);
}
...
function confirmarExclusao(lancamento) {
  const recorrente = Boolean(lancamento.isRecurring || lancamento.recurrenceGroupId);
...
async function carregarMetas(userId, ref, idCategoriaFatura) {
  const [{ data: orcamentos, error: erroOrc }, { data: despesas, error: erroDesp }, { data: compras, error: erroCompras }] = await Promise.all([
    ...
    supabase
      .from('transactions')
      .select('category_id, amount, accounts:account_id(currency)')
      .eq('user_id', userId)
      .eq('type', 'despesa')
      .eq('status', 'pago')
      .gte('date', `${ref}-01`)
...
async function carregarPendentesLista(userId, tipo, inicio, fim, refMes) {
  ...
  const { data, error } = await supabase
    .from('transactions')
    .select('id, type, amount, description, date, account_id, is_recurring, recurrence_group_id, accounts(nome)')
    .eq('user_id', userId)
    .eq('type', tipo)
    .eq('status', 'pendente')
    .gte('date', inicio)
  ...
  if (tipo !== 'despesa') return doConta;
...
async function recarregarCardCartoes() {
  try {
    cacheResumo.cartoes = await carregarCartoesResumo(usuarioAtual.id, refMesString(mesRef));
    renderResumoCards();
  } catch (err) {
    console.error(err);
  }
