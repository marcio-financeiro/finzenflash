...
function renderBalanco({ receitasMes, despesasMes, receitasAnt, despesasAnt }) {
  ...
  if (receitasMes > 0) {
    ...
    poupancaEl.style.color = taxa >= 0 ? 'var(--success)' : 'var(--danger)';
  } else {
    poupancaEl.textContent = '—';
    poupancaEl.style.color = '';
  }
...
// ── Consulta: total de despesa por categoria num mês (reaproveitada pro
// ranking do mês atual e pro cálculo de variação vs mês anterior) ────────
...
// ── Render: Despesas por Categoria + ranking (com variação vs mês anterior,
// só disponível no modo "Mês" — período não tem um "anterior" único) ────
...
// ── Consulta + Render: Evolução do Patrimônio Líquido ────────────────────
// Fonte: patrimony_history, já calculado pelo próprio FinZen (mesma tabela
// usada pela exportação pra IA) — não recalculado aqui.
